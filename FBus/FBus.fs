...
type IBusTransport =
    inherit IDisposable
    abstract Publish: headers:Map<string, string> -> string -> body:ReadOnlyMemory<byte> -> unit
    abstract Send: headers:Map<string, string> -> string -> target:string -> body:ReadOnlyMemory<byte> -> unit
